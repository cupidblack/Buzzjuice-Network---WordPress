...
class TokenController implements TokenControllerInterface {
	...
	public function grantAccessToken( RequestInterface $request, ResponseInterface $response ) {
		...
		if ( ! $grantType instanceof ClientAssertionTypeInterface ) {
			...
			$client_id = $this->clientAssertionType->getClientId();
		}
		...
		if ( $grantType instanceof ClientAssertionTypeInterface ) {
			$client_id = $grantType->getClientId();
		} else {
			// validate the Client ID (if applicable)
			if ( ! is_null( $storedClientId = $grantType->getClientId() ) && $storedClientId != $client_id ) {
		 ...
		 */
		if ( ! $this->clientStorage->checkRestrictedGrantType( $client_id, $grantTypeIdentifier ) ) {
		...
		if ( $requestedScope ) {
			...
			if ( $availableScope ) {
				...
				if ( $clientScope = $this->clientStorage->getClientScope( $client_id ) ) {
		...
		return $grantType->createAccessToken( $this->accessToken, $client_id, $grantType->getUserId(), $requestedScope );
	}
