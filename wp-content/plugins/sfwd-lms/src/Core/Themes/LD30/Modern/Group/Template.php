...
class Template {
	 ...
	 * @return bool
	 */
	private function should_show_header( Views\Group $view, WP_User $user ): bool {
		 ...
		 * @since 4.22.0
		 *
		 * @param bool         $show_header Whether the group header should be shown. Default is false when the user doesn't have access to the Group yet.
		 * @param Views\Group  $view        The group view.
		 * @param WP_User      $user        The user object.
		...
		return apply_filters(
			...
			$group->get_product()->user_has_access( $user ), // The reason is that we always show the progress bar when a user has access to a group.
			$view,
			$user,
			$group
		);
